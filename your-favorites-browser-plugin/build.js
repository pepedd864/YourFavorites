...
const contentOutDir = path.resolve(
  process.cwd(),
  process.env.VITE_CRX_CONTENT_OUTDIR,
)
// 源目录：background script临时生成目录
const backgroundOutDir = path.resolve(
  process.cwd(),
  process.env.VITE_CRX_BACKGROUND_OUTDIR,
)
// 目标目录：Chrome Extension 最终build目录
const outDir = path.resolve(process.cwd(), process.env.VITE_CRX_OUTDIR)
