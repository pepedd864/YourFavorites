...
class UA {
  ...
  getSystemName() {
    const [, $1] =
      this.agent.match(/^[a-z]+\/\d+\.\d+\s?\(([a-z\d\s:;./_-]+)\)/i) || []
    try {
      let osVersion = ''
      if (/Windows/i.test($1)) {
  ...
  getBrowserName() {
    const regexps = Object.keys(this.browserNameMap).map(
      (name) => new RegExp(`${name}(\\/|\\s)(\\d+\\.)+\\d+`),
    )

    // 过滤浏览器信息
    const browsers = this.agent.match(/[a-z\d]+(\/|\s)(\d+\.)+\d+/gi) || []
    let browser = browsers.filter(
      (_) => regexps.findIndex((v) => v.test(_)) !== -1,
    )

    // 如果最后一项不是Safari并且结果长度大于1取最后一项为当前浏览器信息
    browser =
      browser.length > 1 && !/^Safari/.test(browser[browser.length - 1])
        ? browser.reverse()
        : browser
  ...
  _formatBrowserVersion(str: string): Browser {
    try {
      ...
      const { name, version } =
        str.match(/(?<name>[a-z\d]+)(\/|\s)(?<version>(\d+\.)+\d+)/i)?.groups ??
        {}
